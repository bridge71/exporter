<template>
  <div id="app">
    ...
    <el-dialog v-model="showInDialog" title="收款单" width="80%" @close="resetInForm">
      <el-form :model="InForm" label-width="150px" :rules="Rules" ref="InFormRef">
        <!-- 第一行 -->
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="收款单号" prop="ReceNum">
              <el-input v-model="InForm.ReceNum" placeholder="请输入收款单号"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="实际收款日期" prop="RealReceDate">
              <el-date-picker v-model="InForm.RealReceDate" type="date" placeholder="请选择单据日期"></el-date-picker>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="预计收款日期" prop="ExpReceDate">
              <el-date-picker v-model="InForm.ExpReceDate" type="date" placeholder="请选择预计收款日期"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="总金额" prop="TotAmt">
              <el-input v-model="InForm.TotAmt"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="备注" prop="Notes">
              <el-input v-model="InForm.Notes" type="textarea"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            ...
            <el-form-item label="文件名" prop="FileName">
              <el-input v-model="InForm.FileName" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-dialog v-model="showshowInDialog" title="收款单" width="80%" @close="resetInForm">
      <el-form :model="InForm" label-width="150px" :rules="Rules" ref="InFormRef">
        <!-- 第一行 -->
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="收款单号" prop="ReceNum">
              <el-input v-model="InForm.ReceNum" placeholder="请输入收款单号" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="实际收款日期" prop="RealReceDate">
              <el-date-picker v-model="InForm.RealReceDate" type="date" placeholder="请选择单据日期"
          ...
          <el-col :span="12">
            <el-form-item label="预计收款日期" prop="ExpReceDate">
              <el-date-picker v-model="InForm.ExpReceDate" type="date" placeholder="请选择预计收款日期"
                :disabled="true"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="总金额" prop="TotAmt">
              <el-input v-model="InForm.TotAmt" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="币种" prop="CurrencyID">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="备注" prop="Notes">
              <el-input v-model="InForm.Notes" type="textarea" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
const fetchShouldInData = async (ID) => {
  ...
  try {
    ...
    const response = await axios.post('/find/in/shouldIn', params, {
      headers: {
...
const fetchSendData = async (ID) => {
  try {
    ...
    const response = await axios.post('/find/in/send', params, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded', // 设置请求头为表单格式
      },
    })
...
const fetchInData = async () => {
  try {
    const response = await axios.get('/find/in'); // 调用货币数据接口
    InData.value = response.data.In; // 假设返回的数据结构中有 Currency 字段
...
const fetchSaleData = async (ID) => {
  try {
    ...
    const response = await axios.post('/find/in/sale', params, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded', // 设置请求头为表单格式
      },
    })
...
const showshowInDialog = ref(false);
...
const handleEdit = (index, row) => {
  ...
  showInDialog.value = true; // 打开销售订单信息对话框
};

// 查看按钮逻辑
const handleView = (index, row) => {
  InForm.value = { ...row }; // 将当前行的数据赋值给 saleForm
  showshowInDialog.value = true; // 打开查看销售订单信息对话框
};
...
const submitInForm = async () => {
  try {
    ...
    // 添加其他字段
    Object.keys(InForm.value).forEach((key) => {
      formData.append(key, InForm.value[key]);
    });
    ...
    const response = await axios.post('/save/in', formData, {
      headers: {
    ...
    if (response.status === 200) {
      ...
      showInDialog.value = false; // 关闭对话框
      fetchInData(); // 重新获取销售订单信息数据
    } else {
